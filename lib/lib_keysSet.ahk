...
if(!CLSets.global.default_hotkey_scheme)
    ; "capslox" | “capslock_plus” | "capslock_plus_mix"
    CLSets.global.default_hotkey_scheme:="capslock_plus_mix"

if(CLSets.global.default_hotkey_scheme == "capslock_plus") {
    keySchemeInit_capslockPlus()
} else if(CLSets.global.default_hotkey_scheme == "capslox") {
    keySchemeInit_capslox()
} else {
    keySchemeInit_capslockPlusMix()
}
...
keySchemeInit_capslockPlusMix(){
    ...
    if(!keyset.caps_a)
        keyset.caps_a:="keyFunc_backspace"
    if(!keyset.caps_b)
        keyset.caps_b:="keyFunc_openClipboard"
    if(!keyset.caps_c)
        keyset.caps_c:="keyFunc_copy_1"
    if(!keyset.caps_d)
        keyset.caps_d:="keyFunc_saveFile"
    if(!keyset.caps_e)
        keyset.caps_e:="keyFunc_tabPrve"
    if(!keyset.caps_f)
        keyset.caps_f:="keyFunc_searchFile"
    if(!keyset.caps_g)
        keyset.caps_g:="keyFunc_translate_gg"
    if(!keyset.caps_h)
        keyset.caps_h:="keyFunc_jumpLeftWord"
    if(!keyset.caps_i)
    ...
    if(!keyset.caps_m)
        keyset.caps_m:="keyFunc_copyLineDown"
    if(!keyset.caps_n)
        keyset.caps_n:="keyFunc_CopyLineUp"
    if(!keyset.caps_o)
        keyset.caps_o:="keyFunc_end"
    if(!keyset.caps_p)
        keyset.caps_p:="keyFunc_esc"
    if(!keyset.caps_q)
        keyset.caps_q:="keyFunc_qbar"
    if(!keyset.caps_r)
        keyset.caps_r:="keyFunc_tabNext"
    if(!keyset.caps_s)
        keyset.caps_s:="keyFunc_delete"
    if(!keyset.caps_t)
        keyset.caps_t:="keyFunc_translate"
    if(!keyset.caps_u)
    ...
    if(!keyset.caps_v)
        keyset.caps_v:="keyFunc_paste_1"
    if(!keyset.caps_w)
        keyset.caps_w:="keyFunc_closeCurTap"
    if(!keyset.caps_x)
        keyset.caps_x:="keyFunc_cut_1"
    if(!keyset.caps_y)
        keyset.caps_y:="keyFunc_recover"
    if(!keyset.caps_z)
        keyset.caps_z:="keyFunc_undoRedo"
    ...
    if(!keyset.caps_backslash)
        keyset.caps_backslash:="keyFunc_doNothing"
    if(!keyset.caps_semicolon)
        keyset.caps_semicolon:="keyFunc_jumpRightWord"
    if(!keyset.caps_quote)
        keyset.caps_quote:="keyFunc_doubleChar("""""""","""""""")"
    if(!keyset.caps_enter)
        keyset.caps_enter:="keyFunc_enterWherever"
    ...
    if(!keyset.caps_f9)
        keyset.caps_f9:="keyFunc_getJSEvalString"
    if(!keyset.caps_f10)
        keyset.caps_f10:="keyFunc_doNothing"
    if(!keyset.caps_f11)
        keyset.caps_f11:="keyFunc_doNothing"
    ...
    if(!keyset.caps_lalt_a)
        keyset.caps_lalt_a:="keyFunc_jumpBackspace"
    if(!keyset.caps_lalt_b)
        keyset.caps_lalt_b:="keyFunc_drivePop"
    if(!keyset.caps_lalt_c)
        keyset.caps_lalt_c:="keyFunc_copy_2"
    if(!keyset.caps_lalt_d)
        keyset.caps_lalt_d:="keyFunc_doNothing"
    if(!keyset.caps_lalt_e)
        keyset.caps_lalt_e:="keyFunc_doNothing"
    if(!keyset.caps_lalt_f)
        keyset.caps_lalt_f:="keyFunc_doNothing"
    if(!keyset.caps_lalt_g)
        keyset.caps_lalt_g:="keyFunc_doNothing"
    if(!keyset.caps_lalt_h)
        keyset.caps_lalt_h:="keyFunc_selectWordLeft"
    if(!keyset.caps_lalt_i)
        keyset.caps_lalt_i:="keyFunc_UpSelectWord"
    if(!keyset.caps_lalt_j)
        keyset.caps_lalt_j:="keyFunc_LeftSelectWord"
    if(!keyset.caps_lalt_k)
        keyset.caps_lalt_k:="keyFunc_DownSelectWord"
    if(!keyset.caps_lalt_l)
        keyset.caps_lalt_l:="keyFunc_RightSelectWord"
    if(!keyset.caps_lalt_m)
        keyset.caps_lalt_m:="keyFunc_switchSelLowerCase"
    if(!keyset.caps_lalt_n)
        keyset.caps_lalt_n:="keyFunc_switchSelUpperCase"
    if(!keyset.caps_lalt_o)
        keyset.caps_lalt_o:="keyFunc_selectEnd"
    if(!keyset.caps_lalt_p)
        keyset.caps_lalt_p:="keyFunc_selectCurrentWord"
    if(!keyset.caps_lalt_q)
        keyset.caps_lalt_q:="keyFunc_doNothing"
    if(!keyset.caps_lalt_r)
        keyset.caps_lalt_r:="keyFunc_doNothing"
    if(!keyset.caps_lalt_s)
        keyset.caps_lalt_s:="keyFunc_jumpDelete"
    if(!keyset.caps_lalt_t)
        keyset.caps_lalt_t:="keyFunc_doNothing"
    if(!keyset.caps_lalt_u)
        keyset.caps_lalt_u:="keyFunc_selectHome"
    if(!keyset.caps_lalt_v)
        keyset.caps_lalt_v:="keyFunc_paste_2"
    if(!keyset.caps_lalt_w)
        keyset.caps_lalt_w:="keyFunc_CloseCurWindow"
    if(!keyset.caps_lalt_x)
        keyset.caps_lalt_x:="keyFunc_cut_2"
    if(!keyset.caps_lalt_y)
        keyset.caps_lalt_y:="keyFunc_doNothing"
    if(!keyset.caps_lalt_z)
        keyset.caps_lalt_z:="keyFunc_doNothing"
    ...
    if(!keyset.caps_lalt_backslash)
        keyset.caps_lalt_backslash:="keyFunc_doNothing"
    if(!keyset.caps_lalt_semicolon)
        keyset.caps_lalt_semicolon:="keyFunc_selectWordRight"
    if(!keyset.caps_lalt_quote)
        keyset.caps_lalt_quote:="keyFunc_doNothing"
    if(!keyset.caps_lalt_enter)
        keyset.caps_lalt_enter:="keyFunc_enterUpWherever"
    if(!keyset.caps_lalt_comma)
        keyset.caps_lalt_comma:="keyFunc_doNothing"
    if(!keyset.caps_lalt_dot)
        keyset.caps_lalt_dot:="keyFunc_doNothing"
    ...
    if(!keyset.caps_lshift_a)
        keyset.caps_lshift_a:="keyFunc_deleteToLineBeginning"
    if(!keyset.caps_lshift_b)
        keyset.caps_lshift_b:="keyFunc_doNothing"
    if(!keyset.caps_lshift_c)
        keyset.caps_lshift_c:="keyFunc_doNothing"
    ...
    if(!keyset.caps_lshift_n)
        keyset.caps_lshift_n:="keyFunc_doNothing"
    if(!keyset.caps_lshift_o)
        keyset.caps_lshift_o:="keyFunc_doNothing"
    ...
    if(!keyset.caps_lshift_t)
        keyset.caps_lshift_t:="keyFunc_doNothing"
    if(!keyset.caps_lshift_u)
        keyset.caps_lshift_u:="keyFunc_doNothing"
    ...
    if(!keyset.caps_lshift_y)
        keyset.caps_lshift_y:="keyFunc_doNothing"
    if(!keyset.caps_lshift_z)
        keyset.caps_lshift_z:="keyFunc_doNothing"
        
    if(!keyset.caps_lshift_backquote)
        keyset.caps_lshift_backquote:="keyFunc_doNothing"
    if(!keyset.caps_lshift_1)
        keyset.caps_lshift_1:="keyFunc_doNothing"
    if(!keyset.caps_lshift_2)
        keyset.caps_lshift_2:="keyFunc_doNothing"
    if(!keyset.caps_lshift_3)
        keyset.caps_lshift_3:="keyFunc_doNothing"
    if(!keyset.caps_lshift_4)
        keyset.caps_lshift_4:="keyFunc_doNothing"
    if(!keyset.caps_lshift_5)
        keyset.caps_lshift_5:="keyFunc_doNothing"
    if(!keyset.caps_lshift_6)
        keyset.caps_lshift_6:="keyFunc_doNothing"
    if(!keyset.caps_lshift_7)
        keyset.caps_lshift_7:="keyFunc_doNothing"
    if(!keyset.caps_lshift_8)
        keyset.caps_lshift_8:="keyFunc_doNothing"
    if(!keyset.caps_lshift_9)
        keyset.caps_lshift_9:="keyFunc_doNothing"
    if(!keyset.caps_lshift_0)
        keyset.caps_lshift_0:="keyFunc_doNothing"
    if(!keyset.caps_lshift_minus)
        keyset.caps_lshift_minus:="keyFunc_doNothing"
    if(!keyset.caps_lshift_equal)
        keyset.caps_lshift_equal:="keyFunc_doNothing"
    if(!keyset.caps_lshift_backspace)
        keyset.caps_lshift_backspace:="keyFunc_doNothing"
    if(!keyset.caps_lshift_tab)
        keyset.caps_lshift_tab:="keyFunc_doNothing"
    if(!keyset.caps_lshift_leftSquareBracket)
        keyset.caps_lshift_leftSquareBracket:="keyFunc_doNothing"
    if(!keyset.caps_lshift_rightSquareBracket)
        keyset.caps_lshift_rightSquareBracket:="keyFunc_doNothing"
    if(!keyset.caps_lshift_backslash)
        keyset.caps_lshift_backslash:="keyFunc_doNothing"
    if(!keyset.caps_lshift_semicolon)
        keyset.caps_lshift_semicolon:="keyFunc_doNothing"
    if(!keyset.caps_lshift_quote)
        keyset.caps_lshift_quote:="keyFunc_doNothing"
    if(!keyset.caps_lshift_enter)
        keyset.caps_lshift_enter:="keyFunc_doNothing"
    if(!keyset.caps_lshift_comma)
        keyset.caps_lshift_comma:="keyFunc_doNothing"
    if(!keyset.caps_lshift_dot)
        keyset.caps_lshift_dot:="keyFunc_doNothing"
    if(!keyset.caps_lshift_slash)
        keyset.caps_lshift_slash:="keyFunc_doNothing"
    if(!keyset.caps_lshift_space)
        keyset.caps_lshift_space:="keyFunc_doNothing"
    if(!keyset.caps_lshift_ralt)
